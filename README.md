...
Version 0.0.7 keeps the cross-platform wake contract callback-gated while the package remains pre-1.0. Existing strengthening-only remote lock commands remain representable; remote release is a separate, reasoned, audited, time-bounded override and cannot masquerade as a lock command.
...
Publishing is release-driven. A matching `v0.0.*` tag reruns all language gates and publishes the internal JavaScript package to GitHub Packages at `@thehypertextstudio/curfew-protocols`; no public npm registry is used. The publication runner is macOS because its prepublish gate verifies the generated Swift package with CryptoKit. Swift Package Manager consumes the same immutable release tag.
