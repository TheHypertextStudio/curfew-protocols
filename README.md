...
```swift
.package(url: "https://github.com/TheHypertextStudio/curfew-protocols", exact: "2.0.0")
```
...
The generated JVM artifact uses package `studio.hypertext.curfew.protocols` and Maven coordinates `studio.hypertext.curfew:curfew-protocols:2.0.0`. The Android application ID remains the separate reverse-DNS identifier `studio.hypertext.curfew`.

```kotlin
implementation("studio.hypertext.curfew:curfew-protocols:2.0.0")
```
