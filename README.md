...
- Desk presence is optional on a status publication, is one of `present`, `away`, or `unknown`, and carries its own observation instant. The device fuses camera person-detection with HID idle locally and publishes only that verdict; raw sensor signals never cross the wire. A publisher that omits `presence` is reporting nothing about presence, which is not the same as `unknown`.
