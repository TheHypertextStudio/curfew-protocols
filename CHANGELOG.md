...
- Quote the GitHub Packages tag/version assertion correctly so a matching immutable tag reaches publication.
...
- Publish npm and Kotlin Maven artifacts in independent jobs so registry authorization failures cannot suppress the other platform artifact.
...
- A distinct first-party OAuth resource for native Curfew clients with exact account, device, entitlement, encrypted-sync, and wake scopes. Native default grants intentionally carry no remote-unlock authority.
- Kotlin/JVM codegen under package `studio.hypertext.curfew.protocols`, Maven coordinates `studio.hypertext.curfew:curfew-protocols:0.2.0`, a Gradle build, and native shared-vector tests.
- Release automation that verifies TypeScript, Swift, and Kotlin before publishing npm through trusted OIDC and Kotlin through GitHub Packages; Swift Package Manager uses the matching immutable release tag.
...
- This is a pre-1.0 minor release with intentionally breaking wire changes: wake-enabled days replace their editable fixed unlock with a campaign release authority, and account-enabled consumers gain encrypted record/version requirements.
...
- Account enrollment carries separate encryption and signing public keys plus proof-of-possession, never a plaintext device name, platform label, or application version.
- JSON request proofs bind an RFC 8785 canonical body digest; an embedded top-level device proof is omitted from that digest to avoid circular signing while covering every enrollment field.

### Device-presence milestone
